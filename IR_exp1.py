# -*- coding: utf-8 -*-
"""
Created on Fri Sep 18 08:14:13 2020
...
def tokenize_tweet(document):
    document = document.lower()  #统一为小写
    a = document.index("username")  #返回第一次出现索引
    b = document.index("clusterno")  
    c = document.rindex("tweetid") - 1  #最后一次出现的索引
    ...
    stopwords = [line.strip() for line in codecs.open(r"F:\\Dataset\\stop_words_eng.txt")]
    result = []#空列表
    for word in terms:
        expected_str = Word(word)
        expected_str = expected_str.lemmatize("v")#单词变体还原
        if expected_str not in uselessTerm:#去无用单词
            if expected_str not in stopwords:  #去除停用词
...
def get_postings():
    global postings
    f = open(r"F:\Dataset\tweets.txt")

    lines = f.readlines()  # 读取全部内容
    mylog = open(r"F:\Dataset\Inverted2.txt", mode='w', encoding='utf-8')
    mylog2 = open(r"F:\Dataset\preprocessed.txt", mode='w', encoding='utf-8')
...
def do_rankSearch(terms):
    ...
    for item in terms:
        if item in postings:
            for tweetid in postings[item]: #统计词频
                if tweetid in Answer:
                    Answer[tweetid] += 1
                else:
                    Answer[tweetid] = 1   
    Answer = sorted(Answer.items(), key=lambda asd: asd[1], reverse=True) #按照词频排序
    return Answer

def token(doc):
    doc = doc.lower()#转小写
    terms = TextBlob(doc).words.singularize()  #分词

    result = []
    for word in terms:
        expected_str = Word(word)
        expected_str = expected_str.lemmatize("v")  #词干化 动词
        result.append(expected_str)
    return result
